...
def _extract_from_pdf_url(url: str):
    ...
    raw_text = "\n\n".join(texts)
    num_pages = len(pdf)
    pages_with_text = sum(1 for l in lengths if l >= 20)
    if num_pages == 0:
        tipo_extracao = "PDF_TEXTUAL"
        observacoes.append("PDF vazio ou sem páginas.")
    else:
        ...
        if ratio >= 0.5:
            ...
            observacoes.append("PDF provavelmente escaneado. OCR não aplicado ou incompleto.")
...
def _extract_from_html_url(url: str):
    ...
    if texto:
        ...
        paginas_estimadas = 0
        observacoes.append("Nenhum conteúdo textual significativo encontrado no HTML.")
...
def extrair_conteudo(analise: dict):
    """
    Recebe a análise de um link e extrai o conteúdo textual e metadados.
    Suporta PDF direto, HTML com links para PDF e HTML textual.
    Retorna um dicionário com os documentos extraídos e indicação do próximo agente.
    """
    ...
    if tipo_conteudo == "PDF_DIRETO":
        ...
        if not pdfs:
            observacoes.append("tipo_conteudo=HTML_COM_PDF mas pdfs_encontrados está vazio.")
        for item in pdfs:
        ...
        observacoes.append(f"tipo_conteudo inválido ou não suportado: {tipo_conteudo}")
        tipo_extracao_global = "HTML"
    resultado = {
        "fonte": fonte,
