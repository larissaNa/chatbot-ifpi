...
from apps.core.prompt import get_web_answer_prompt
...
def responder_web(question: str, *, max_results: int = 4) -> dict[str, Any]:
    ...
    for s in sources:
        rendered_lines.append(f"- {s.get('title')} - {s.get('url')}")
