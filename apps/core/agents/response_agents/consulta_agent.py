...
from ...prompt import get_rag_answer_prompt
...
def retrieve_with_threshold(
    ...
    *,
    ...
    # Fallback/combinação com API alternativa
    ...
    # Normalização de relevância e combinação
    ...
    # Dedup por id+hash conteúdo, pegando maior relevância
    seen_map: dict[str, tuple[Any, float]] = {}
    for doc, r in combined:
        doc_id = getattr(doc, "id", None) or getattr(doc, "metadata", {}).get("chunk_id") or str(hash(getattr(doc, "page_content", "")))
        prev = seen_map.get(str(doc_id))
        if not prev or r > prev[1]:
            seen_map[str(doc_id)] = (doc, r)

    docs_with_scores = list(seen_map.values())

    filtered = [(d, s) for d, s in docs_with_scores if s >= float(score_threshold)]
    filtered.sort(key=lambda x: x[1], reverse=True)

    # Se nada passou no threshold mas houve resultados, usa os top-k
    if not filtered and docs_with_scores:
        filtered = sorted(docs_with_scores, key=lambda x: x[1], reverse=True)[:k]
...
def _render_answer(answer: str, sources: list[dict[str, Any]]) -> str:
    ...
    lines = [f"Resposta:\n{a}", "", "Fontes:"]
    for s in sources:
        title = (s.get("title") or "Documento Oficial").strip()
